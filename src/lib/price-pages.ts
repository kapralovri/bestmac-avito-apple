 ...
 * `chore: update prices [skip ci]`. Коммит помечен [skip ci], поэтому git-
 * интеграция Vercel сама НЕ пересобирает — свежий SSG-HTML даёт Vercel Deploy
 * Hook, который тот же workflow дёргает после пуша (секрет VERCEL_DEPLOY_HOOK_URL).
 * Страницы читают JSON на этапе сборки, отдельный cron для них не нужен.
 *
