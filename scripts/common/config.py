...
#
# GST-74: было 3 → стало 1. Капча на Avito решается практически на КАЖДОЙ
# загрузке страницы (статистика RuCaptcha: ~16 решений на прогон при ровно
# 16 загрузках), то есть каждая лишняя страница — это живые деньги. Выдача
# отсортирована по новизне, а прогон идёт каждые 30 минут: за это время
# 50 лотов первой страницы не набегает, страницы 2–3 были чистым расходом.
# Медиана от этого почти не страдает — основной источник рынка это база цен
# и накопитель коллектора (intake-raw-prices.json), живая выдача лишь запасной.
SCAN_PAGES_PER_FAMILY = _envi("SCAN_PAGES_PER_FAMILY", 1)
...
# 0.62 = цена ниже 62% медианы (скидка >38%) считается подозрительной.
SCAM_FLOOR = _envf("SCAM_FLOOR", 0.62)

# Целевая выкупная цена = BUYOUT_FACTOR * медиана рынка (Москва).
BUYOUT_FACTOR = _envf("BUYOUT_FACTOR", 0.80)

# Авито Доставка работает до этой суммы — региональные (не-Москва) лоты дороже
# выкупить удалённо нельзя, поэтому в метку 📦 доставка их не берём.
DELIVERY_MAX_PRICE = _envi("DELIVERY_MAX_PRICE", 150_000)
