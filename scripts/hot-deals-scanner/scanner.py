...
class AvitoScanner:
    ...
    def notify(self, title, price, market_low, buyout, ram, ssd, url, cycles, urgent, avito_low_price=False):
        if not TELEGRAM_URL: return
        status = "🚨 <b>СРОЧНО!</b> " if urgent else ""
        if cycles and cycles < 150: status += "🔋 <b>АКБ ИДЕАЛ!</b> "
        if avito_low_price: status += "📉 <b>Авито: Ниже рынка!</b>"
    ...
    def run(self):
        ...
        logger.info(f"🔎 Найдено объявлений: {total_items}")
        ...
        for item in items:
            try:
                ...
                # ПРОВЕРКА БЕЙДЖА "НИЖЕ РЫНОЧНОЙ"
                badge_text = item.get_text().lower()
                is_avito_low_price = "ниже рыночной" in badge_text or "цена ниже" in badge_text
                ...
                # УСЛОВИЕ: Либо наша математика, либо пометка Авито
                is_match = False
                market_low = 0
                buyout = 0
                ...
                if matched_stat:
                    ...
                    buyout = matched_stat['buyout_price']
                    if price <= int(market_low * PRICE_THRESHOLD_FACTOR):
                        is_match = True
                
                # Если Авито пометил как выгодное — шлем в любом случае (если модель опознана)
                if is_avito_low_price and matched_stat:
                    is_match = True

                if is_match:
                    logger.info(f"🔥 MATCH: {price} руб. (Badge: {is_avito_low_price})")
                    cycles, urgent = self.deep_analyze(raw_url)
                    self.notify(raw_title, price, market_low, buyout, ram, ssd, url, cycles, urgent, is_avito_low_price)
                    self.seen.add(url)
                    new_ads_found += 1
            except: continue
