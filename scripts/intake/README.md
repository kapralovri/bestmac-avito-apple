...
4. Нажми **«▶️ Запустить мониторинг (4 вкладки)»** — откроются 4 вкладки Авито,
   каждая сама обновляется раз в ~75 сек и шлёт новые лоты. Останавливать —
   кнопкой **«⏹ Остановить мониторинг»**, иначе сторож откроет вкладки заново.
...
## Слежение за моделью по вашей цене (v1.7.0)

Поиск конкретной модели дешевле нужной суммы делает **домашний браузер**, а не
VPS — это нулевой расход капчи (на сервере капча решается почти на каждой
загрузке страницы, см. `docs/captcha-budget-GST-74.md`).

Как это работает:

1. В боте: `/модель` → семья → модель → конфигурация → **«🔔 Следить за этой
   конфигурацией»** → называете предельную цену.
2. Бот отдаёт готовую ссылку: та же выдача Avito, но с `pmax` (ваш потолок) и
   `s=104` (новые сверху — расширение читает только первую страницу).
3. Открываете ссылку в браузере с расширением и оставляете вкладку висеть.
   Сторож теперь держит и такие вкладки: выгруженную разбудит, замолчавшую
   вернёт на нужный URL. Закрытую **не** воскрешает — закрыли, значит хотели.
4. Расширение шлёт новые лоты на сервер, тот сверяет с подпиской (конфигурация
   и цена) и присылает подходящие в Telegram. Цена перепроверяется на сервере,
   поэтому алерты остаются верными, даже если Avito однажды перестанет понимать
   `pmax` в ссылке.

В попапе такие вкладки помечены `🔔` с указанием потолка цены.

## Сторож вкладок (v1.6.0)
Раньше сбор мог тихо встать через несколько часов: Chrome выгружал фоновую
вкладку из памяти, Avito подсовывал капчу или редирект уводил вкладку за
пределы masks — и перезапустить цикл изнутри страницы было уже некому.

Теперь поверх вкладок работает сторож в service worker (`chrome.alarms`,
раз в минуту). Каждая вкладка после прохода шлёт ему пульс; если пульса нет
дольше 4 минут — вкладка возвращается на свой URL мониторинга, выгруженная
будится, закрытая открывается заново. Капча больше не выглядит как «просто
0 карточек»: в попапе появляется ошибка, плюс системное уведомление.

В попапе под статусом видно состояние каждой из 4 вкладок:
`✅` живёт • `⚠️` молчит • `💤` выгружена Chrome • `🤖` капча • `❌` закрыта.

## Проверка
- В попапе расширения растёт «Отправлено всего: N».
- Там же — пульс каждой вкладки (должен обновляться раз в ~1–2 минуты).
...
- Если коллектор замолчит дольше 25 минут, бот сам напишет в Telegram
  (ночью, 23:00–09:00, не будит — Mac штатно спит). Пороги переопределяются
  через `COLLECTOR_SILENT_MIN`, `COLLECTOR_COOLDOWN_MIN`, `COLLECTOR_QUIET_FROM`,
  `COLLECTOR_QUIET_TO`.
